...
def copy_pdf(src_folder, name, dest_folder):
    """Kopiert PDF falls vorhanden"""
    src_file = os.path.join(src_folder, name + ".pdf")
...
def strecken_copy(base_folder):
    ...
    with open(logfile, "w", encoding="utf-8") as log:
        ...
        for ds in strecken_daten:
            ...
            os.makedirs(strecke_folder, exist_ok=True)
            write(f"\n➡️ Strecke {nr}")

            # Funktion-FOP
            ok = copy_pdf(os.path.join(base_folder, FOP_FOLDER), ds["fop"], strecke_folder)
            write(f"   📑 FOP: {ds['fop']} {'✅ kopiert' if ok else '❌ fehlt'}")

            # AOP
            if ds["aop"] != "NULL":
                ok = copy_pdf(os.path.join(base_folder, SR_FOLDER), ds["aop"], strecke_folder)
                write(f"   🔌 AOP: {ds['aop']} {'✅ kopiert' if ok else '❌ fehlt'}")

            # ABP-Start
            ok = copy_pdf(os.path.join(base_folder, SK_FOLDER), ds["abp_start"], strecke_folder)
            write(f"   ▶️ ABP-Start: {ds['abp_start']} {'✅ kopiert' if ok else '❌ fehlt'}")

            # ABP-Ziel
            ok = copy_pdf(os.path.join(base_folder, SK_FOLDER), ds["abp_ziel"], strecke_folder)
            write(f"   🎯 ABP-Ziel: {ds['abp_ziel']} {'✅ kopiert' if ok else '❌ fehlt'}")
