...
# Konfiguration
# --------------------------------------------------------------------
APP_ID = "com.nextcloud.desktopclient.nextcloud"   # Flatpak-ID des Nextcloud-Clients
DEFAULT_USER = "kleif"
...
def check_write(path: Path, filename=".nextcloud_lock_test") -> bool:
    try:
...
def ensure_dir(path: Path):
    if not path.exists():
        ...
        run(["mkdir", "-p", str(path)], sudo=True, check=True, capture=False)
...
def remount_exfat_vfat(mp: Path, src: str, uid: int, gid: int):
    ...
    cmd = [
        "mount", "-t", "exfat",
        "-o", f"uid={uid},gid={gid},umask=022,iocharset=utf8",
        src, str(mp)
    ]
    cmd_preview(["sudo"] + cmd)
    run(cmd, sudo=True, check=False, capture=False)
...
def test_target(mountpoint: Path):
    section("Schreibtest auf Ziel")
    print(f"{I['folder']} {C.BOLD}{mountpoint}{C.RESET}")
    ls_ld(mountpoint)
    mt = findmnt(mountpoint)
    if mt:
        print(mt)
    if check_write(mountpoint):
        ok("Schreibtest erfolgreich")
    else:
        fail("Kein Schreibzugriff")

def flatpak_overrides(mountpoint: Path):
    section("Flatpak-Rechte für Nextcloud")
    ...
    cmds = [
        ["flatpak", "override", "--user",
         f"--filesystem={str(mountpoint)}", APP_ID],
        ["flatpak", "override", "--user",
         "--filesystem=xdg-run/gvfs", APP_ID],
    ]
    for c in cmds:
        cmd_preview(c)
        run(c, check=False, capture=False)

    ok("Flatpak-Overrides gesetzt (sofern Flatpak & Nextcloud installiert).")

def restart_nextcloud():
    section("Nextcloud-Client neu starten")
    if not which("flatpak"):
    ...
    if rc.returncode != 0:
        info(f"Nextcloud-Client ({APP_ID}) nicht installiert – überspringe.")
        return

    # Falls läuft: leise beenden
    subprocess.run(
        ["flatpak", "kill", APP_ID],
        stdout=subprocess.DEVNULL,
        stderr=subprocess.DEVNULL
    ...
    try:
        ...
        ok("Nextcloud-Client gestartet.")
    except Exception as e:
        warn(f"Konnte Nextcloud-Client nicht starten: {e}")
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Fix Nextcloud (Flatpak) Berechtigungen für /mnt/backup mit hübscher Ausgabe"
    )
    p.add_argument("--user", default=DEFAULT_USER,
                   help=f"Zielnutzer (Default: {DEFAULT_USER})")
    p.add_argument("--mount", default=DEFAULT_MOUNT,
                   help=f"Mountpunkt (Default: {DEFAULT_MOUNT})")
    return p.parse_args()
...
def main():
    ...
    kv("Nextcloud-App-ID", APP_ID)

    test_target(mountpoint)

    section(f"Reparatur {I['hammer']}")
    ensure_dir(mountpoint)
    fixed = fix_permissions(mountpoint, user, uid, gid)
    if not fixed:
        warn("Konnte Berechtigungen nicht sauber reparieren – trotzdem weiter mit Tests.")

    section(f"Lock-Test {I['lock']}")
    lock = mountpoint / ".nextcloud_write_test"
    try:
        with open(lock, "w") as f:
            f.write("lock\n")
        ok("Lock-/Test-Datei kann geschrieben werden.")
        lock.unlink(missing_ok=True)
    except Exception as e:
        fail(f"weiterhin kein Schreibzugriff auf {mountpoint}: {e}")

    flatpak_overrides(mountpoint)
    restart_nextcloud()
