...
# =========================
# Ausgabeziel:
# True  -> schreibe in Unterordner ./_md_scan relativ zur CWD
...
AGG_FILENAME    = "allsummary.md"   # nur bei CONTENT_MODE == "full"
# Inhaltserfassung im Bericht: "none" | "snippet" | "full"
CONTENT_MODE = "snippet"
SNIPPET_CHARS = 800
TOC_DEPTH = 3
# Welche Text-/Code-Dateien sollen gescannt werden? Hinweis: Groß-/Kleinschreibung der Endungen wird ignoriert.
# ".txt", ".py", ".html"
FILE_TYPES = [".py"]
FILE_TYP = FILE_TYPES
...
def make_snippet(text: str, max_chars: int = 600):
    ...
    for raw in text.splitlines():
        ...
        acc.append(s)
        if sum(len(x)+1 for x in acc) >= max_chars: break
...
def tree_listing_for_root(root: Path, include_only_md=True, exclude_names=None):
    ...
    def walk_dir(path: Path, depth: int):
        ...
        dirs = sorted([e for e in entries if e.is_dir()
                       and e.name not in exclude_names
                       and is_within_root(e, root)], key=lambda p: p.name.lower())
        files = sorted([e for e in entries if e.is_file()
                        and (not include_only_md or e.suffix.lower() in {ext.lower() for ext in FILE_TYPES})],
...
def truncate_name(name: str, width: int) -> str:
    if width <= 0 or len(name) <= width: return name
...
def scan_md_files(root: Path, scan_exclude_dirs, exclude_exact_files):
    results = []
    dir_count = file_count = md_count = 0
    skipped_dirs = set()
    ...
    for dirpath, dirnames, filenames in os.walk(root, followlinks=False):
        ...
        for d in sorted(dirnames):
            ...
            if d in scan_exclude_dirs:
                if d not in skipped_dirs: skipped_dirs.add(d)
                continue
        ...
        for fn in filenames:
            ...
            if is_wanted_file(fn):
                # bestimmte MD-Dateien explizit ausschließen (z. B. Aggregat)
                if Path(dirpath, fn).resolve() in exclude_exact_files:
                    continue
                md_count += 1
                p = Path(dirpath)/fn
                if not is_within_root(p, root): 
                ...
                results.append({
                    ...
                    "snippet": snippet,
                    "toc": toc,
                    "content": text if CONTENT_MODE == "full" else None,
                })

                if PROGRESS_EVERY and (md_count % PROGRESS_EVERY == 0):
                    print(f"{IC_INFO} Zwischenstand: {md_count} Markdown-Dateien, {dir_count} Verzeichnisse.")

    dur = time.time() - start
    print(f"{IC_DONE} Scan fertig in {dur:.2f}s — Verzeichnisse: {dir_count}, Dateien: {file_count}, Markdown: {md_count}")
    if skipped_dirs:
...
def path_with_icons(rel_path: str) -> str:
    ...
    if not parts: return "."
    if len(parts) == 1: return f"{FILE_ICON} {parts[0]}"
...
def write_report(root: Path, outdir: Path, md_files):
    ...
    report_lines.append(f"Einstellungen: content={CONTENT_MODE}, snippet_chars={SNIPPET_CHARS}, toc_depth={TOC_DEPTH}, types={', '.join(FILE_TYPES)}")
    report_lines.append("")
    ...
    for parent in sorted(by_parent.keys(), key=lambda s: (s != "", s.lower())):
        ...
        for it in by_parent[parent]:
            ...
            report_lines.append(f"     Überschriften: {it['headings']}, Zeilen: {it['lines']}, Wörter: {it['words']}, Zeichen: {it['chars']}")
            if it.get("toc"):
    ...
    report_lines.append("")
    report_lines.append(tree_listing_for_root(root, include_only_md=True))
    report_lines.append("")
    report_lines.append("=== Ordnerbaum (Ausgabeordner) ===")
    report_lines.append("")
    report_lines.append(tree_listing_for_root(outdir, include_only_md=False))
    report_lines.append("")
...
def print_usage():
    msg = """
Verwendung:
  summary.py [--.EXT ...] [--full|--snippet|--none] [--help]
...
def parse_cli_args(argv):
    """
    Unterstützt Flags wie --.py, --.md sowie --full/--snippet/--none.
    Gibt (types, content_mode) zurück, wobei:
...
def main():
    ...
    # --- NEU: CLI lesen und globale Defaults ggf. überschreiben ---
    cli_types, cli_content_mode = parse_cli_args(sys.argv[1:])

    # Globale Konfiguration anpassen (damit alle Hilfsfunktionen die neuen Werte sehen)
    global FILE_TYPES, CONTENT_MODE
    ...
    print(f"  {IC_FILE} Gefundene Text-/Code-Dateien: {len(md_files)}")
