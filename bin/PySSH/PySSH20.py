...
# Optional: Passwort-Datei + Umschalter für Passwort-Login
SSH_PW_FILE  = getattr(config, "SSH_PW_FILE", None)
USE_PASSWORD = getattr(config, "USE_PASSWORD", False)
...
# Pfade für Key- und (optional) Passwort-Datei vorbereiten
SSH_KEY_PATH = os.path.join(SSH_DIR, SSH_KEY_FILE) if SSH_KEY_FILE else None
SSH_PW_PATH  = os.path.join(SSH_DIR, SSH_PW_FILE) if SSH_PW_FILE else None
...
def main():
    ...
    if USE_PASSWORD:
        print("   🔐 Auth     : Passwort-Login (kein SSH-Key)\n")
    else:
    ...
    if USE_PASSWORD:
        ...
        print("🖥️ Starte SSH-Verbindung (Passwort-Login) ...")
        print(f"   ➜ Verbinde zu {SSH_USER}@{SSH_HOST} ohne Key\n")
    else:
        cmd = [
            "ssh",
            "-i", SSH_KEY_PATH,
