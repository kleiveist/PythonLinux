...
class ExamConfig:
    max_points: int
    pass_percent: int
    grade_thresholds: List[Tuple[int,str,int]]  # (Grenze%, Icon, Note)

CONFIGS: Dict[str, ExamConfig] = {
    "90": ExamConfig(
        max_points=90,
        pass_percent=50,
        grade_thresholds=[(90,"🔵",1),(80,"🟢",2),(70,"🟡",3),(50,"🟠",4),(45,"🔴",5)],
    ),
    "45": ExamConfig(
        max_points=45,
        pass_percent=50,
...
def parse_args():
    parser = argparse.ArgumentParser(description="PyObis Exam: 45- oder 90-Punkte-Schema wählen.")
...
# ===== Bewertung =====
def status_icon(p:int, cfg:ExamConfig):  return ("✅","Bestanden") if p>=cfg.pass_percent else ("❌","Nicht bestanden")
def grade_icon(p:int, cfg:ExamConfig):
    for t,icon,note in cfg.grade_thresholds:
...
def build_result(total:int, cfg:ExamConfig):
    percent = round((total / cfg.max_points) * 100) if cfg.max_points>0 else 0
    if total==0 and percent==0:
        return f'Ergebnis: 0 | {I_BEGIN} Nicht begonnen\n', 'Prozent: 0% | ⚪ 0\n', percent
    si,st = status_icon(percent, cfg)
    gi,gn = grade_icon(percent, cfg)
...
def process_file(path:Path, session_dir:Path, base_dir:Path, cfg:ExamConfig):
    ...
    print(f"   {I_CALC}  Summe: {total}/{cfg.max_points} → {percent}%")
